...
import type { ScanSignal, BreakoutSignal } from '../hooks/useScanner';

type TabId = 'trend-pullback' | 'breakout' | 'consolidation';
...
function BreakoutResultsTable({ signals }: { signals: BreakoutSignal[] }) {
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm" aria-label="Breakout scan results">
        <thead>
          <tr className="border-b border-dashboard-border">
            <th className="text-left py-2 px-2 text-xs text-dashboard-muted font-medium">
              Symbol
            ...
            <th className="text-center py-2 px-2 text-xs text-dashboard-muted font-medium">
              Touches
            </th>
            <th className="text-center py-2 px-2 text-xs text-dashboard-muted font-medium">
              Vol ✓
        ...
        <tbody>
          {signals.map((signal, idx) => (
            ...
            >
              ...
              <td className="py-2.5 px-2 text-right font-mono text-dashboard-muted">
                ₹{signal.levelValue.toFixed(2)}
              </td>
              <td className="py-2.5 px-2 text-right font-mono text-dashboard-text">
                ₹{signal.breakoutPrice.toFixed(2)}
              </td>
              <td className="py-2.5 px-2 text-right font-mono text-loss">
                {signal.trailingStopLoss != null
                  ? `₹${signal.trailingStopLoss.toFixed(2)}`
                  : `₹${signal.initialStopLoss.toFixed(2)}`}
              </td>
              <td className="py-2.5 px-2 text-center font-mono text-dashboard-text">
                {signal.touchCount}
              </td>
              <td className="py-2.5 px-2 text-center">
                {signal.volumeConfirmed ? (
                  <span className="text-profit" aria-label="Volume confirmed">✓</span>
                ) : (
                  <span className="text-dashboard-muted" aria-label="Volume not confirmed">—</span>
                )}
              </td>
            </tr>
          ))}
...
function BreakoutTab() {
  const { breakoutSignals, isBreakoutScanning, scanError, lastBreakoutScanTime, triggerBreakoutScan } =
    useScanner();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-medium text-dashboard-text">
            Multi-Touch Breakout Scanner
          </h3>
          <p className="text-xs text-dashboard-muted mt-0.5">
            Detect structural breakouts with volume confirmation and ATR trailing stop
          </p>
        </div>
        <Button
          onClick={triggerBreakoutScan}
          isLoading={isBreakoutScanning}
          disabled={isBreakoutScanning}
          aria-label="Trigger breakout scan"
        >
          {isBreakoutScanning ? 'Scanning...' : 'Scan'}
        </Button>
      </div>
      ...
      {isBreakoutScanning && (
        <Card className="flex items-center justify-center py-8" role="status" aria-live="polite">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-dashboard-muted">
              Scanning for multi-touch breakout patterns...
            </p>
          </div>
        </Card>
      )}
      ...
      {/* No results */}
      {!isBreakoutScanning && !scanError && breakoutSignals.length === 0 && lastBreakoutScanTime && (
        <Card>
          <div className="text-center py-6">
            <p className="text-sm text-dashboard-muted">No breakout setups found</p>
            <p className="text-xs text-dashboard-muted mt-1">
              Last scanned: {new Date(lastBreakoutScanTime).toLocaleTimeString()}
            </p>
          </div>
        </Card>
      )}
      ...
      {/* Results */}
      {!isBreakoutScanning && breakoutSignals.length > 0 && (
        <Card padding="none">
          <div className="px-4 pt-4 pb-2 flex items-center justify-between">
            <h4 className="text-sm font-medium text-dashboard-text">
              {breakoutSignals.length} breakout{breakoutSignals.length !== 1 ? 's' : ''} detected
            </h4>
            {lastBreakoutScanTime && (
              <span className="text-xs text-dashboard-muted">
                {new Date(lastBreakoutScanTime).toLocaleTimeString()}
              </span>
            )}
          </div>
          <BreakoutResultsTable signals={breakoutSignals} />
        </Card>
      )}
    </div>
  );
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        >
          ...
          {activeTab === 'breakout' && <BreakoutTab />}
