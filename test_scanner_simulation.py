#!/usr/bin/env python3
"""
Scanner Simulation Test - Simulates trading hours to show detailed analysis
"""
import sys
import logging
import pandas as pd
...
def test_with_simulation():
    """Test scanner with simulated trading hours"""
    print("=" * 70)
    print("SCANNER SIMULATION TEST (During Trading Hours)")
    print("=" * 70)
    print()
    
    # Temporarily override time check for testing
    original_check = strategy_engine.check_market_time
    
    def mock_market_time():
        """Mock function that always returns True"""
        return True, "Time OK (Simulated)"
    
    strategy_engine.check_market_time = mock_market_time
    
    print(f"Current Time: {datetime.datetime.now().strftime('%H:%M:%S IST')}")
    print(f"Trading Window: {config.START_TIME} - {config.END_TIME}")
    print(f"⚠️  Simulating trading hours for testing...")
    print()
    ...
    for symbol in config.WATCHLIST:
        ...
        if ok:
            ...
            print(f"      ❌ {msg}")
            print(f"         - Price vs VWAP: {last['close']:.2f} vs {last['vwap']:.2f} ({'Below' if last['close'] < last['vwap'] else 'Above'})")
            print(f"         - EMA Trend: {last['ema_20']:.2f} vs {prev['ema_20']:.2f} ({'Down' if last['ema_20'] < prev['ema_20'] else 'Up'})")
            print(f"         - MACD: {last['macd']:.4f} ({'Below 0' if last['macd'] < 0 else 'Above 0'})")
            print(f"         - Price vs EMA: {last['close']:.2f} vs {last['ema_20']:.2f} ({'Below' if last['close'] < last['ema_20'] else 'Above'})")
        ...
        if ok:
            ...
            print(f"         - Looking for: Large body (>1.5x avg) + High volume + Red candle")
        ...
        if ok:
            ...
            print(f"         - Break Check: {last['close']:.2f} < {prev['low']:.2f} = {last['close'] < prev['low']}")
        ...
        print("\n5️⃣ Summary:")
        print(f"   Market Trend: {'Bearish' if last['close'] < last['vwap'] and last['macd'] < 0 else 'Bullish/Neutral'}")
        print(f"   Price Position: {'Below VWAP' if last['close'] < last['vwap'] else 'Above VWAP'}")
        print(f"   EMA Trend: {'Down' if last['ema_20'] < prev['ema_20'] else 'Up'}")
        print(f"   MACD Signal: {'Bearish' if last['macd'] < 0 else 'Bullish'}")
    ...
    print()
    print("📝 Notes:")
    print("   - This test simulates trading hours to show full analysis")
    print("   - Daily data is used (not 5-minute candles)")
    print("   - Strategy requires very specific conditions to trigger")
    print("   - No signal is normal when conditions aren't met")
    print()
